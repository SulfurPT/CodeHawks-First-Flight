...
contract PuppyRaffle is ERC721, Ownable {
    ...
    // @audit Question: Can no one enter?
    function enterRaffle(address[] memory newPlayers) public payable {
        require(msg.value == entranceFee * newPlayers.length, "PuppyRaffle: Must send enough to enter raffle");
        for (uint256 i = 0; i < newPlayers.length; i++) {
            players.push(newPlayers[i]);
        }
        // @audit Issue: This is a gas intensive way to check for duplicates, could lead to DoS
    ...
    // @audit What about player that are on the index 0?
    ...
    function selectWinner() external {
        ...
        // @audit its possible to predict the winner
        uint256 winnerIndex =
        ...
        // @audit overflow nad unsafe canst of uint256 to uint64
        totalFees = totalFees + uint64(fee);

        uint256 tokenId = totalSupply();

        // We use a different RNG calculate from the winnerIndex to determine rarity
        // @audit its possible to predict the rarity
